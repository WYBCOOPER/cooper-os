...
function captureRoute(text, proj) {
  ...
  if (/(明天|今天|今晚|明日|上午|下午)/.test(text)) {
    if (focus.length >= 3) {
      todos.unshift({ t: text, done: false, pri: '#4f8dff' });
  ...
  // 规则3：其他 → 待办
  const tag = proj ? '📁' + proj + ' ' : '';
  todos.unshift({ t: tag + text, done: false, pri: '#4f8dff' });
  store.set('cg_todos', todos); renderTodos();
...
let todos = store.get('cg_todos', [
  ...
  { t: 'Python 学习 1 小时', done: false, pri: '#4f8dff' },
...
function addTodo() {
  ...
  todos.unshift({ t:v, done:false, pri:'#4f8dff' });
...
function maybeShowReview() {
  ...
  if (h >= 21 || h < 3) {  // 21:00 - 3:00
    if (!reviewDoneToday[dateKey()]) {
      ...
      setTimeout(() => document.getElementById('review-modal').classList.add('open'), 800);
    }
  }
}
...
setTimeout(maybeShowReview, 5000); // 延迟到所有变量初始化后;
...
function subToTodo(si, i) {
  ...
  todos.unshift({ t: '🧩 ' + projTag + text, done: false, pri: '#4f8dff' });
  store.set('cg_todos', todos); renderTodos();
  splits[si].subs[i] = '';
  ...
  track.forEach(d => {
    ...
    stat.days.forEach(dd => {
      cells += '<span style="display:inline-block;width:16px;height:16px;border-radius:4px;margin-right:3px;' +
        (dd.n > 0 ? 'background:var(--accent);' : 'background:rgba(96,140,255,0.12);') +
...
let projects = store.get('cg_projects', [
  { id: 1, name: 'COOPER OS', type: '软件', version: '10.2.1', ddl: '', status: '进行中', progress: 70, links: { ddl: false, todo: true, track: false, split: true, trackKey: '', splitIdx: '' } },
