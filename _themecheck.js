...
function dateKey() { const n=new Date(); return `${n.getFullYear()}
...
function delFocus(i) { focus.splice(i,1); saveFocus(); renderFocus(); }
function addFocus() {
  // 绑定追踪选项（从 track 生成）
...
function saveReview() {
  ...
  if (tomorrow) {
    ...
    if (lines.length) { focus = lines.map(t => ({ t, done: false })); saveFocus(); renderFocus(); }
  }
...
function trackAdd(key) {
  ...
  if (linked) { saveFocus(); renderFocus(); }
  renderTrack();
}
